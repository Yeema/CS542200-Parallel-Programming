...
{   double start = (double)clock(), end,latency=0, startIo , endIo,io = 0, startComm, endComm, comm = 0;
    ...
    while (!sorted) {
        ...
        if (numtasks > 1){
            //even
            //1 even communication sort: even時 rank內部數量是odd 最後一個element會落單 可增加平行度
            if (isodd(rankSize) && rank<numtasks){
                float buffer;
                MPI_Status status;
            ...
            //odd
            //1 odd communication sort: even時 rank內部數量是odd 最後一個element會落單 可增加平行度
            // odd時 也會有人落單
            if(iseven(rankSize) && rank<numtasks){
                float buffer_odd,buffer_even;
                MPI_Status status;
                if (rank+1 < numtasks){
                    timing(&startComm,rank);
                    MPI_Send(&nums[count-1],1,MPI_FLOAT,rank+1,0,MPI_COMM_WORLD);
                    MPI_Recv(&buffer_odd,1,MPI_FLOAT,rank+1,1,MPI_COMM_WORLD,&status);
                    timing(&endComm,rank);
                    aggrTiming(&comm, startComm , endComm, rank);
                    if(nums[count-1] > buffer_odd){
                        nums[count-1] = buffer_odd;
                        sorted = 0;
                    }
                }
                if(rank > 0){
                    timing(&startComm,rank);
                    MPI_Send(&nums[0],1,MPI_FLOAT,rank-1,1,MPI_COMM_WORLD);
                    MPI_Recv(&buffer_even,1,MPI_FLOAT,rank-1,0,MPI_COMM_WORLD,&status);
                    timing(&endComm,rank);
                    aggrTiming(&comm, startComm , endComm, rank);
                    if(nums[0] < buffer_even){
                        nums[0] = buffer_even;
                        sorted = 0;
                    }
                }
            }
            else if(rank<numtasks && isodd(rankSize)){
                float buffer;
                MPI_Status status;
                if(isodd(rank)){
                    if(rank+1<numtasks){
